...
def test_build_query_syntax_no_params():
...
real_params = {
    'media_type': [Literal('Video', datatype=XSD.string), Literal('Audio', datatype=XSD.string)],
...
def test_build_query_syntax_params_individual(params):
    """Test syntax of generated queries by querying on rdflib Graph instance with query parameters."""
    g = Graph()
    q = build_query(**{params[0]: params[1]})
    qres = g.query(q, initNs=namespaces)
    assert qres.bindings == [{}]


def test_build_query_syntax_params_all():
    """Test syntax of generated queries by querying on rdflib Graph instance with query parameters."""
    g = Graph()
    q = build_query(**real_params)
    qres = g.query(q, initNs=namespaces)
    assert qres.bindings == [{}]


def test_build_query_syntax_fail():
    g = Graph()
