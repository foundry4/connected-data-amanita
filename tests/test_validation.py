"""Pytest Suite to test validation tools. Full coverage over validation.py through testing of process_list_content_query_params."""
import pytest

from app.utils import processquery
from app.utils.processquery import process_list_content_query_params
...
def test_process_query_params_multi_allowed(monkeypatch, mapping):
    ...
    monkeypatch.setattr(processquery, '_get_validators', lambda _: mapping)
...
def test_process_query_params_multi_not_allowed(monkeypatch, param_multidict):
    ...
    monkeypatch.setattr(processquery, '_get_validators', lambda _: get_multi_query_parameter_test_mappings_restricted())
...
def test_process_query_params_invalid_parameter(monkeypatch):
    ...
    monkeypatch.setattr(processquery, '_get_validators', lambda _: get_single_query_parameter_test_mappings())
    with pytest.raises(InvalidInputQuery):
