...
def test_get_content(monkeypatch):
    ...
    expected_response_file = open("test_clients_elastic/data/processed_output_100_examples.json")
    expected_response = {
...
def test_get_content_no_results(monkeypatch):
    raw_response_file = open("test_clients_elastic/data/no_hits.json")
    raw_response = json.load(raw_response_file)
...
def test_get_similar(monkeypatch):
    ...
    expected_response_file = open("test_clients_elastic/data/processed_output_100_similar_examples.json")
    expected_response = {
        'results': json.load(expected_response_file)
    }
    assert mapped_response == expected_response
...
def test_get_item(monkeypatch):
    ...
    expected_response_file = open("test_clients_elastic/data/processed_output_item_example.json")
